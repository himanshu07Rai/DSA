...
class Solution
{
    ...
    Node *copyList(Node *head)
    {
        ...
        Node *clonedHead = NULL,*clonedTail = NULL;
        Node* temp = head;
        while(temp)
        {
            insert(temp->data,clonedHead,clonedTail);
            temp = temp->next;
        }
        ...
        {
            if(ogNode->next)
                ogNode->next->arb =ogNode->arb?ogNode->arb->next:ogNode->arb;
            ogNode = ogNode->next->next;
        }
        
        ogNode = head;
        clone = clonedHead;
        // cout<<ogNode->data<<" "<<clonedHead->data;
        while(ogNode)
        {
            ogNode->next = clone->next;
            ogNode = ogNode->next;
            if(ogNode)
                clone->next = ogNode->next;
            clone = clone->next;
        }
        return clonedHead;
